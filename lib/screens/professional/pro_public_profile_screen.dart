...
class ProPublicProfileScreen extends ConsumerWidget {
  final String uid;

  const ProPublicProfileScreen({super.key, required this.uid});
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('Como os clientes me veem', style: TextStyle(color: AppTheme.textPrimary, fontSize: 16)),
        centerTitle: true,
      ),
      body: proAsync.when(
        data: (proData) {
        ...
        error: (err, stack) => const Center(child: Text('Erro ao carregar perfil.')),
      ),
    );
  }
